...
#define LOG_GROUP    NFC_LOG_GROUP_GENERAL
		...
		const char* nfc_property_name[15] = {
		  "NP_TIMEOUT_COMMAND",
		  "NP_TIMEOUT_ATR",
		  "NP_TIMEOUT_COM",
		  "NP_HANDLE_CRC",
				...
				nfc_target* pnt)
		{
			...
			uint8_t* abtTmpInit = (uint8_t*)malloc(sizeof(uint8_t) * maxAbt);
			size_t  szInit = 0;
			int res;
		...
		*/
			...
			nfc_target_receive_bits(nfc_device* pnd, uint8_t* pbtRx, const size_t szRx, uint8_t* pbtRxPar)
		{
		 ...
		 const nfc::sErrorMessage sErrorMessages[13] = {
			...
			nfc_strerror(const nfc_device* pnd)
		{
			...
			size_t  i;
			for (i = 0; i < (sizeof(sErrorMessages) / sizeof(struct nfc::sErrorMessage)); i++) {
		...
		//size_t nfc_device_validate_modulation(nfc_device* pnd, const nfc_mode mode, const nfc_modulation* nm)
		//{
		//	 size_t res;
