...
{
	nfc_context* context = (nfc_context*)malloc(sizeof(nfc_context));
	...
	for (int i = 0; i < MAX_USER_DEFINED_DEVICES; i++) {
		strcpy(context->user_defined_devices[i].name, "");
		strcpy(context->user_defined_devices[i].connstring, "");
		context->user_defined_devices[i].optional = false;
	}
	context->user_defined_device_count = 0;

#ifdef ENVVARS
	// Load user defined device from environment variable at first
	char* envvar = getenv("LIBNFC_DEFAULT_DEVICE");
	if (envvar) {
		strcpy(context->user_defined_devices[0].name, "user defined default device");
		strncpy(context->user_defined_devices[0].connstring, envvar, NFC_BUFSIZE_CONNSTRING);
		context->user_defined_devices[0].connstring[NFC_BUFSIZE_CONNSTRING - 1] = '\0';
	...
	if (envvar) {
		strcpy(context->user_defined_devices[0].name, "user defined device");
		strncpy(context->user_defined_devices[0].connstring, envvar, NFC_BUFSIZE_CONNSTRING);
		context->user_defined_devices[0].connstring[NFC_BUFSIZE_CONNSTRING - 1] = '\0';
	...
	string_as_boolean(envvar, &(context->allow_autoscan));

	// Load "intrusive scan" option
	envvar = getenv("LIBNFC_INTRUSIVE_SCAN");
	string_as_boolean(envvar, &(context->allow_intrusive_scan));

	// log level
	envvar = getenv("LIBNFC_LOG_LEVEL");
	if (envvar) {
		context->log_level = atoi(envvar);
	}
#endif // ENVVARS
	...
	// Debug context state
#if defined DEBUG
	log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_NONE, "log_level is set to %"PRIu32, context->log_level);
#else
	log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "log_level is set to %"PRIu32, context->log_level);
#endif
	log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "allow_autoscan is set to %s", (context->allow_autoscan) ? "true" : "false");
	log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "allow_intrusive_scan is set to %s", (context->allow_intrusive_scan) ? "true" : "false");

	log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "%d device(s) defined by user", context->user_defined_device_count);
	for (uint32_t i = 0; i < context->user_defined_device_count; i++) {
		log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "  #%d name: \"%s\", connstring: \"%s\"", i, context->user_defined_devices[i].name, context->user_defined_devices[i].connstring);
	}
	return context;
}
...
connstring_decode(const nfc_connstring connstring, const char* driver_name, const char* bus_name, char** pparam1, char** pparam2)
{
	...
	param0 = (char*)malloc(n * sizeof(char));
	if (param0 == NULL) {
		perror("malloc");
		return 0;
	}
	char* param1 = NULL;
	 param1 = (char*)malloc(n * sizeof(char)); 
	if (param1 == NULL) {
		perror("malloc");
		free(param0);
		return 0;
	...
	char* param2 = NULL;
	param2 = (char*)malloc(n * sizeof(char));
	if (param2 == NULL) {
		perror("malloc");
		free(param0);
		free(param1);
